...
import { leaguePda, membershipPda, standingsPda } from "./program.js";
...
export type SettlementVerdict =
  | {
      ...
      readonly paid: readonly boolean[];
      readonly winners: readonly string[];
    }
  | { readonly ok: false; readonly reason: "NOT_SETTLED" };

/**
 * Confirm a league's final standings have been frozen on-chain.
 *
 * Settlement is the one piece of the payout flow the program cannot verify for
 * itself — the winners come from an off-chain score feed (issue #28). So the
 * server reads the `FinalStandings` account back rather than trusting a client
 * report: it exists (the settle authority posted it), and we surface which legs
 * of the payout have already been paid so the UI can show progress.
 */
export async function verifySettlement(
  ...
  const league = leaguePda(leagueId);
  const address = standingsPda(league);

  const account = await program.account["finalStandings"]?.fetchNullable(address);
  if (!account) return { ok: false, reason: "NOT_SETTLED" };

  const raw = account as {
    league: PublicKey;
    winners: readonly PublicKey[];
    feePaid: boolean;
  ...
  return {
    ...
    feePaid: raw.feePaid,
    paid: [...raw.paid],
    winners: raw.winners.map((w) => w.toBase58()),
  };
}
